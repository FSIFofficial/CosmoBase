...
    try {
      // データをGoogleフォームに送信（no-corsを使うのがポイント！）
...
      })
      
      // 送信成功（no-corsの場合、エラーが出なければ成功とみなす）
      setIsSuccess(true)
    } catch (error) {
      console.error(error)
...
                        className="w-full bg-transparent border border-[#83CBEB]/30 rounded p-3 text-[#EEEEFF] focus:border-[#83CBEB] focus:outline-none transition-colors"
                        placeholder="宇宙 太郎"
                      />
                    </div>
...
                        className="w-full bg-transparent border border-[#83CBEB]/30 rounded p-3 text-[#EEEEFF] focus:border-[#83CBEB] focus:outline-none transition-colors resize-none"
